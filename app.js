...
if (!Meteor.isClient) {
  ...
  angular.module('when2hack').controller('EventDetailsCtrl', function ($scope, $meteor, $stateParams, $rootScope) {
    ...
    $scope.startSelection = function (startTimeUnit) {
      ...
      $scope.availTimesTemp = [startTimeUnit];

      var idx = $scope.availTimes.indexOf(startTimeUnit);
      $scope.selectionType = idx >= 0 ? false : true;
    };
    $scope.endSelection = function () {
      $scope.selectionStarted = false;
      if ( $scope.selectionType )
        $scope.availTimes = _.union($scope.availTimes, $scope.availTimesTemp);
      else
        $scope.availTimes = _.without($scope.availTimes, $scope.availTimesTemp);
    };
    $scope.updateEndTime = function (timeUnit) {
      if (!$scope.selectionStarted) return;
      $scope.endTime = timeUnit;
      if ($scope.startTime < $scope.endTime) {
        $scope.availTimesTemp = _.filter($scope.timeUnits, function (unit) {
          return unit >= $scope.startTime && unit <= $scope.endTime;
        });
      }
      else
        $scope.availTimesTemp = _.filter($scope.timeUnits, function (unit) {
    ...
    $scope.cellClass = function (timeUnit) {
      ...
      var idx2 = $scope.availTimesTemp.indexOf(timeUnit);
      if ( $scope.selectionStarted ) {
        if ( $scope.selectionType ) {
          if (idx >= 0 || idx2 >= 0) return "color-available";
        }
        else {
          if (idx >= 0 || idx2 < 0) return "color-available";
        }
        ...
        if (idx >= 0 ) return "color-available";
      }
      return "";
    };
  });
